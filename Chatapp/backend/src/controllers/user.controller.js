...
export async function getRecommendedUsers(req, res) {
    try {
        ...
        console.log("reco", recommendedUsers);
...
export async function acceptFriendRequest(req, res) {
    try {
        ...
        //Checking if the current user is the actual recipient
        if (friendRequest.recipient.toString() != req.user.id) {
            return res.status(403).json({ message: "You are not authorised to accept the request" });
        }
        ...
        console.log("Error in acceptFriendRequest constroller", error.message);
...
export async function getFriendRequests(req, res) {
    try {
        ...
        console.log("Error in getPendingFriendRequests controller\n", error.message);
        res.status(500).json({ message: "Internal Server Error" });
    }
}
